...
#        if verbose >= 1:
#            traceback.print_exc() 
        exceptioninfo = traceback.format_exception_only(sys.exc_info()[0], sys.exc_info()[1]) #@UndefinedVariable
        exceptiontext = ""
...
def _runBuiltIn(app, config, mode):
    ...
    try:
        from . import ext_wsgiutils_server
        if config["verbose"] >= 2:
...
def run():
    ...
    for e in config["ext_servers"]:
        fn = SUPPORTED_SERVERS.get(e)
        if fn is None:
            print("Invalid external server '%s'. (expected: '%s')" % (e, "', '".join(list(SUPPORTED_SERVERS.keys()))))
